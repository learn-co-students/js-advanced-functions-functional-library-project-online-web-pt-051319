  ...
  return {
    ...
    map: function(collection, fcn) {
      ...
      for (let i = 0; i < collection.length; i++) {
        newCollection.push(fcn(collection[i]));
      }
      return newCollection;
    },

    reduce: function(collection, fcn, startingPoint) {
      let destructCollection = collection.slice();
      let value = startingPoint || destructCollection.shift();

      for (let i = 0; i < destructCollection.length; i++) {
        value = fcn(value, destructCollection[i]);
      }
      return value;
    },

    find: function(collection, fcn) {
      for (let i = 0; i < collection.length; i++) {
        if (fcn(collection[i])) {
          return collection[i];
        }
    ...
    uniq: function(collection, isSorted, callback) {
      ...
      if (callback) {
        ...
        collection.map(item => {
          ...
          if (
            iteratedValues.filter(element => element === iteratedValue)
              .length == 1
          ) {
